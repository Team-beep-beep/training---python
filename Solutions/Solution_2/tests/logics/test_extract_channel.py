...
class TestConfig(pydantic.BaseModel):
    module_config: ExtractChannel.Config
...
class TestClass:
    @pytest.mark.parametrize("test_config", ["test_extract_channel_config"], indirect=True)
    def test_extract_channel(self, test_config: TestConfig):
        ...
        test_output = np.fromfile(test_config.test_signal_path, dtype=np.complex64)
